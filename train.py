...
def training(nfiltbank, orderLPC):
    ...
    directory = os.getcwd() + '/Train' #return curenent folder 
    fname = str()
    #Testing with speaker 1 to 8 in Train folders
...
def training1(nfiltbank, orderLPC):
    nSpeaker = 8
    nCentroid = 16
    codebooks_mfcc = np.empty((nSpeaker,nfiltbank,nCentroid))
    codebooks_lpc = np.empty((nSpeaker, orderLPC, nCentroid))
